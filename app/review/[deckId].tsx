...
export default function ReviewScreen() {
  ...
  const rate = useCallback(async (rating: ReviewRating) => {
    ...
    try {
      ...
      if (index + 1 >= queue.length) router.replace({ pathname: '/review/result', params: { deckId, deckTitle: deck?.title || 'Bộ từ', ...Object.fromEntries(Object.entries(nextSummary).map(([key, value]) => [key, String(value)])) } });
  ...
  return <AppScreen scroll={false} contentStyle={styles.screen}>
    ...
    <Animated.View {...panResponder.panHandlers} style={[styles.cardArea, { transform: position.getTranslateTransform() }]}>
      <Pressable onPress={toggleFlip} style={styles.pressable}>
        <Animated.View style={[styles.flashcard, styles.face, { transform: [{ perspective: 1000 }, { rotateY: frontRotate }] }]}><Text style={styles.faceLabel}>TỪ VỰNG</Text><Pressable accessibilityRole="button" accessibilityLabel={`Phát âm ${card.term}`} hitSlop={10} onPress={(event) => { event.stopPropagation(); speakEnglish(card.term).catch(() => undefined); }} style={styles.speakerButton}><Ionicons name="volume-high" size={26} color={colors.primary} /></Pressable><Text style={styles.term}>{card.term}</Text>{card.pronunciation ? <Text style={styles.pronunciation}>/{card.pronunciation}/</Text> : null}<View style={styles.tapHint}><Text style={styles.hint}>Chạm để xem nghĩa</Text></View></Animated.View>
        <Animated.View style={[styles.flashcard, styles.face, styles.back, { transform: [{ perspective: 1000 }, { rotateY: backRotate }] }]}><Text style={styles.faceLabel}>NGHĨA</Text><Pressable accessibilityRole="button" accessibilityLabel={`Phát âm ${card.term}`} hitSlop={10} onPress={(event) => { event.stopPropagation(); speakEnglish(card.term).catch(() => undefined); }} style={styles.speakerButton}><Ionicons name="volume-high" size={26} color={colors.primary} /></Pressable>{card.imageUrl ? <Image source={{ uri: card.imageUrl }} style={styles.image} contentFit="cover" /> : null}<Text style={styles.meaning}>{card.meaning}</Text>{card.example ? <Text style={styles.example}>“{card.example}”</Text> : null}<Text style={styles.hint}>Bạn nhớ từ này thế nào?</Text></Animated.View>
      </Pressable>
    </Animated.View>
